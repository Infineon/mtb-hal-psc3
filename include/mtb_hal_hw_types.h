...
typedef struct
{
    ...
    cy_en_hppass_dac_mode_t             dac_mode;
    #endif
 ...
 * and/or HAL releases.
 */
typedef void* mtb_hal_nvm_configurator_t;
...
typedef struct
{
    ...
    #else // if (MTB_HAL_DRIVER_AVAILABLE_LPTIMER)
    void* empty;
...
typedef struct
{
    // Workaround for PSC3 personality conflict
    #if (MTB_HAL_DRIVER_AVAILABLE_LPTIMER) || defined (CY_DEVICE_PSC3)
    #if (defined (CY_IP_MXS40SRSS) && (CY_IP_MXS40SRSS_VERSION >= 2))
    MCWDT_Type*                         base;
    #else
    MCWDT_STRUCT_Type*                  base;
    #endif
    #else // (MTB_HAL_DRIVER_AVAILABLE_LPTIMER)
    void* empty;
    #endif /* (MTB_HAL_DRIVER_AVAILABLE_LPTIMER) */
